...
def test_create_app() -> None:
    ...
    # There is no route that lists the lexicon, by design.
    check("/v1/lexicon/browse" not in paths, "no browse route is exposed")
...
def test_edit_takes_effect() -> None:
    """A saved reading must change what the engine says on the very next call.

    The engine memoises its routing decision per token. A reviewer always hears
    a word before deciding it is wrong, so the old reading is already cached by
    the time the edit lands: the save returned 200, the file was written, and
    the voice went on saying the same thing. That is the bug this guards.
    """
    print("an edit changes the next reading")
    calls = {"cleared": 0}
    ...
    g2p = _fake_g2p(gold={_lexicon_key("צוריק"): {
        "word": "צוריק", "ipa_primary": "ʦirˈik", "variants": ["ʦirˈik"],
        "layer": "G", "freq": 99, "note": ""}})
    g2p._ROUTE_CACHE = Cache({"צוריק": "stale"})

    lexicon_edits.apply_edit_row(g2p, {
        "word": "צוריק", "ipa_primary": "ʦurˈik", "layer": "G", "note": ""})

    check(calls["cleared"] >= 1, "the route cache is cleared on write")
    check(not g2p._ROUTE_CACHE, "no stale routing decision survives")
    check(g2p.GOLD_LEXICON[_lexicon_key("צוריק")]["ipa_primary"] == "ʦurˈik",
          "the new reading is what the engine now holds")
